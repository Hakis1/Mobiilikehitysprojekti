...
suspend fun main() = Korge(width = 512, height = 512, bgcolor = Colors["#2b2b2b"]) {
    ...
    val solidRect = solidRect(width = 1000.0, height = 40.0, color = Colors.GREEN)
    val solidBase = solidRect(width = 1000.0, height = 200.0, color = Colors.SANDYBROWN)
    ...
    addUpdater { dt ->
        ...
        if (keys[Key.UP]) player.y -= 256 * dt.milliseconds / 1000.0
        if (keys[Key.UP]){
            konaSound()
        }
